...
class TestSlotBulkCreate:
    def test_verified_specialist_can_bulk_create_slots(self, verified_specialist):
        payload = {"start_times": [future_dt(24), future_dt(25)]}
        resp = verified_specialist.client.post(SLOTS_BULK_URL, payload, format="json")
        assert resp.status_code == 201
        assert len(resp.data) == 2
        assert AvailabilitySlot.objects.filter(specialist=verified_specialist.profile).count() == 2
    ...
    def test_unverified_specialist_cannot_create_slots(self, unverified_specialist):
    ...
    def test_anonymous_cannot_create_slots(self, anon_client):
        resp = anon_client.post(SLOTS_BULK_URL, {"start_times": [future_dt(24)]}, format="json")
    ...
    def test_duplicate_start_times_in_same_request_are_rejected(self, verified_specialist):
        t = future_dt(24)
        resp = verified_specialist.client.post(
...
class TestSlotDelete:
    def test_specialist_can_delete_own_unbooked_slot(self, verified_specialist):
    ...
    def test_specialist_cannot_delete_booked_slot(self, verified_specialist, user_with_profile):
        slot = make_slot(verified_specialist.profile)
        Appointment.objects.create(
            slot=slot, specialist=verified_specialist.profile, user_profile=user_with_profile.profile
        )
        slot.is_booked = True
        slot.save()
        resp = verified_specialist.client.delete(f"{SLOTS_URL}{slot.id}/")
    ...
    def test_another_specialist_cannot_delete_foreign_slot(self, verified_specialist, db):
        slot = make_slot(verified_specialist.profile)
...
@pytest.mark.django_db
class TestSlotList:
    def test_anonymous_user_sees_only_future_unbooked_slots(self, verified_specialist, anon_client):
...
class TestAppointmentCreate:
    def test_user_with_profile_can_book_a_slot(self, verified_specialist, user_with_profile):
        slot = make_slot(verified_specialist.profile)
        resp = user_with_profile.client.post(APPOINTMENTS_URL, {"slot": slot.id}, format="json")
        assert resp.status_code == 201
        assert Appointment.objects.count() == 1
        slot.refresh_from_db()
        assert slot.is_booked is True

    def test_user_without_profile_cannot_book(self, verified_specialist, user_without_profile):
        slot = make_slot(verified_specialist.profile)
        resp = user_without_profile.client.post(APPOINTMENTS_URL, {"slot": slot.id}, format="json")
        assert resp.status_code == 400
    ...
    def test_specialist_cannot_book_a_slot(self, verified_specialist, db):
        ...
        other_profile = make_specialist_profile(other_user, verified=True)
        other_client = make_client_for(other_user)
        resp = other_client.post(APPOINTMENTS_URL, {"slot": slot.id}, format="json")
        assert resp.status_code == 400

    def test_already_booked_slot_cannot_be_booked_again(self, verified_specialist, user_with_profile):
        slot = make_slot(verified_specialist.profile)
        slot.is_booked = True
        slot.save()
        resp = user_with_profile.client.post(APPOINTMENTS_URL, {"slot": slot.id}, format="json")
    ...
    def test_booking_nonexistent_slot_returns_400(self, user_with_profile):
        resp = user_with_profile.client.post(APPOINTMENTS_URL, {"slot": 99999}, format="json")
        assert resp.status_code == 400

    def test_anonymous_user_cannot_book(self, verified_specialist, anon_client):
        slot = make_slot(verified_specialist.profile)
        resp = anon_client.post(APPOINTMENTS_URL, {"slot": slot.id}, format="json")
        assert resp.status_code == 401
...
class TestAppointmentList:
    def test_user_sees_only_their_own_appointments(self, verified_specialist, user_with_profile):
        slot = make_slot(verified_specialist.profile)
    ...
    def test_user_does_not_see_another_users_appointments(self, verified_specialist, user_with_profile, db):
        slot = make_slot(verified_specialist.profile)
        appointment = Appointment.objects.create(
            slot=slot, specialist=verified_specialist.profile, user_profile=user_with_profile.profile
        )
        ...
        other_appt = Appointment.objects.create(
            slot=other_slot, specialist=verified_specialist.profile, user_profile=other_profile
        )
    ...
    def test_specialist_sees_only_their_appointments(self, verified_specialist, user_with_profile):
        slot = make_slot(verified_specialist.profile)
        appointment = Appointment.objects.create(
            slot=slot, specialist=verified_specialist.profile, user_profile=user_with_profile.profile
        )
    ...
    def test_admin_sees_all_appointments(self, verified_specialist, user_with_profile, admin_client):
        slot = make_slot(verified_specialist.profile)
        appointment = Appointment.objects.create(
            slot=slot, specialist=verified_specialist.profile, user_profile=user_with_profile.profile
        )
        ...
        other_sp_profile = make_specialist_profile(other_sp_user, verified=True)
        other_slot = make_slot(other_sp_profile, hours_ahead=48)
        other_appt = Appointment.objects.create(
            slot=other_slot, specialist=other_sp_profile, user_profile=other_profile
        )
...
class TestAppointmentReschedule:
    ...
    def test_cannot_reschedule_to_slot_booked_concurrently(self, db):
        ...
        other_profile = make_profile(other_user)
        Appointment.objects.create(
            slot=self.new_slot, specialist=self.sp.profile, user_profile=other_profile
        )
...
class TestAppointmentCancel:
    ...
    def test_specialist_can_cancel_appointment(self):
        resp = self.sp.client.patch(self._cancel_url(), {"status": "cancelled"}, format="json")
        assert resp.status_code == 200
        self.appointment.refresh_from_db()
    ...
    def test_wrong_status_value_is_rejected(self):
        resp = self.sp.client.patch(self._cancel_url(), {"status": "completed"}, format="json")
        assert resp.status_code == 400

    def test_user_cannot_cancel_appointment(self):
        resp = self.user.client.patch(self._cancel_url(), {"status": "cancelled"}, format="json")
        assert resp.status_code == 403
    ...
    def test_other_specialist_cannot_cancel_foreign_appointment(self, db):
        ...
        make_specialist_profile(other_user, verified=True)
        other_client = make_client_for(other_user)
        resp = other_client.patch(self._cancel_url(), {"status": "cancelled"}, format="json")
        assert resp.status_code == 404

    def test_cancelled_slot_can_be_booked_again(self, db):
        self.sp.client.patch(self._cancel_url(), {"status": "cancelled"}, format="json")

        other_user = make_user(email="other@test.com")
        other_profile = make_profile(other_user)
        other_client = make_client_for(other_user)
        resp = other_client.post(APPOINTMENTS_URL, {"slot": self.slot.id}, format="json")
    ...
    def test_cancelled_slot_has_multiple_appointments(self, db):
        ...
        other_profile = make_profile(other_user)
        other_client = make_client_for(other_user)
        other_client.post(APPOINTMENTS_URL, {"slot": self.slot.id}, format="json")
...
class TestAutoCompletion:
    def test_past_confirmed_appointment_becomes_completed_on_list(
        ...
        appt = make_appointment(slot, verified_specialist.profile, user_with_profile.profile)
    ...
    def test_multiple_past_appointments_all_completed(
        ...
        appt1 = make_appointment(slot1, verified_specialist.profile, user_with_profile.profile)
        appt2 = make_appointment(slot2, verified_specialist.profile, user_with_profile.profile)
...
class TestCompletedAppointmentsUser:
    def test_completed_list_returns_only_completed_appointments(
        ...
        make_appointment(past_slot, verified_specialist.profile, user_with_profile.profile)
        make_appointment(future_slot, verified_specialist.profile, user_with_profile.profile)

        resp = user_with_profile.client.get(COMPLETED_URL)

        assert resp.status_code == 200
        assert len(resp.data["results"]) == 1
        assert resp.data["results"][0]["status"] == Appointment.Status.COMPLETED

    def test_completed_appointment_includes_book_again_url(
        self, verified_specialist, user_with_profile
    ):
        slot = make_past_slot(verified_specialist.profile)
        make_appointment(slot, verified_specialist.profile, user_with_profile.profile)
        ...
        assert f"specialist={verified_specialist.profile.id}" in appt["book_again_url"]
...
class TestAppointmentSorting:
    ...
    def setup(self, verified_specialist, user_with_profile):
        ...
        self.appt_24h = Appointment.objects.create(
            slot=self.slot_24h, specialist=self.sp.profile, user_profile=self.user.profile
        )
        self.appt_48h = Appointment.objects.create(
            slot=self.slot_48h, specialist=self.sp.profile, user_profile=self.user.profile
        )
        self.appt_72h = Appointment.objects.create(
            slot=self.slot_72h, specialist=self.sp.profile, user_profile=self.user.profile
        )
    ...
    def test_confirmed_sort_date_asc(self):
        resp = self.user.client.get(APPOINTMENTS_URL + "?sort_field=date&sort_direction=asc")
        ids = [a["id"] for a in resp.data["results"]]
    ...
    def test_confirmed_sort_date_desc(self):
        resp = self.user.client.get(APPOINTMENTS_URL + "?sort_field=date&sort_direction=desc")
        ids = [a["id"] for a in resp.data["results"]]
    ...
    def test_confirmed_invalid_sort_direction_falls_back_to_asc(self):
        resp = self.user.client.get(APPOINTMENTS_URL + "?sort_field=date&sort_direction=random")
        assert resp.status_code == 200
        ids = [a["id"] for a in resp.data["results"]]
        assert ids.index(self.appt_24h.id) < ids.index(self.appt_72h.id)

    def test_confirmed_unknown_sort_field_falls_back_to_date(self):
        resp = self.user.client.get(APPOINTMENTS_URL + "?sort_field=price&sort_direction=asc")
        assert resp.status_code == 200

    def test_specialist_confirmed_sort_date_desc(self):
        resp = self.sp.client.get(APPOINTMENTS_URL + "?sort_field=date&sort_direction=desc")
        ids = [a["id"] for a in resp.data["results"]]
    ...
    def test_completed_sort_date_asc(self):
        ...
        appt_3h = make_appointment(past_slot_3h, self.sp.profile, self.user.profile)
        appt_6h = make_appointment(past_slot_6h, self.sp.profile, self.user.profile)
        appt_9h = make_appointment(past_slot_9h, self.sp.profile, self.user.profile)

        resp = self.user.client.get(COMPLETED_URL + "?sort_field=date&sort_direction=asc")
    ...
    def test_completed_sort_date_desc(self):
        ...
        resp = self.user.client.get(COMPLETED_URL + "?sort_field=date&sort_direction=desc")
    ...
    def test_completed_specialist_sort_date_desc(self):
        ...
        resp = self.sp.client.get(COMPLETED_URL + "?sort_field=date&sort_direction=desc")
...
class TestAppointmentDateFilter:
    ...
    def setup(self, verified_specialist, user_with_profile):
        ...
        self.appt_june10 = Appointment.objects.create(
            slot=self.slot_june10, specialist=self.sp.profile, user_profile=self.user.profile
        )
        self.appt_june11 = Appointment.objects.create(
            slot=self.slot_june11, specialist=self.sp.profile, user_profile=self.user.profile
        )
        self.appt_july01 = Appointment.objects.create(
            slot=self.slot_july01, specialist=self.sp.profile, user_profile=self.user.profile
        )
    ...
    def test_confirmed_filter_with_sorting(self):
        date_str = self.slot_june10.start_time.date().isoformat()
        resp = self.user.client.get(f"{APPOINTMENTS_URL}?date={date_str}&sort_direction=desc")
        assert resp.status_code == 200
        ids = [a["id"] for a in resp.data["results"]]
    ...
    def test_completed_filter_by_date(self):
        ...
        appt_1 = make_appointment(past_slot_1, self.sp.profile, self.user.profile)
        appt_2 = make_appointment(past_slot_2, self.sp.profile, self.user.profile)
    ...
    def test_completed_filter_with_sorting(self):
        ...
        resp = self.user.client.get(f"{COMPLETED_URL}?date={date_str}&sort_direction=asc")
